...
import (
	...
	"hangar.bpfx.org/otto/otto2/ottonet"
	"hangar.bpfx.org/otto/otto2/ottotf"
...
func main() {
	...
	flag.StringVar(&xmlfilename, "x", "", "XML File")
	...
	flag.Parse()
	if *debug {
		os.Setenv("OTTODEBUG", "1")
	} else {
		os.Setenv("OTTODEBUG", "0")
	}

	if xmlfilename == "" {
		support.ErrorIt("No XML File Given", 3)
	}

	if templatefilename == "" {
		support.ErrorIt("No Template File Given", 4)
	}

	support.DebugIt("Debug: true")

	debugenabled = *debug

	tmpdir, err := ioutil.TempDir("ottotmp", "ottotmp")
	if err != nil {
		support.ErrorIt("Temp Dir Error: "+err.Error(), 6)
	}
	defer os.RemoveAll(tmpdir)

	if esxuser == "" {
		fmt.Print("Enter Username: ")
		byteUser, err := terminal.ReadPassword(int(syscall.Stdin))
		if err != nil {
			fmt.Println("\nError User")
		}
	...
	if err != nil {
		support.ErrorIt("Opening XML File |"+xmlfilename+"| "+err.Error(), 2)
	} else {
	...
	*/
	...
	for i := 0; i < len(ServerList); i++ {
		...
		support.DebugIt(networkname.Name)
		//TODO: Fix Hardcoded
		...
		outfile = tmpdir + "/" + ServerList[i].ServerName + ".tf"
		var ipaddy string
		ipaddy = fmt.Sprintf("172.18.37.20%d", i)
		support.DebugIt(ipaddy)
		/*
		...
		if err != nil {
			support.ErrorIt("Writing the TF "+err.Error(), 5)
		}
		ottotf.RunTF(outfile, ServerList[i].ServerName+".tf", esxuser, esxpass, tmpdir)
	}
}
